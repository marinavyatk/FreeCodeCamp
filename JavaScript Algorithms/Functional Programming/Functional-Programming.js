...
function add(listOfBooks, bookName) {
  let doubledListOfBooks = listOfBooks.slice();
  doubledListOfBooks.push(bookName);
  return doubledListOfBooks;
}
...
function remove(listOfBooks, bookName) {
  ...
  let doubledListOfBooks = listOfBooks.slice();
  if (book_index >= 0) {
    doubledListOfBooks.splice(book_index, 1);
    return doubledListOfBooks;
  }
}
...
Array.prototype.myMap = function (callback) {
  const newArray = [];
  this.forEach((el, index, array) => newArray.push(callback(el, index, array)));
  return newArray;
};
...
function getRating(watchList) {
  ...
  let averageRating =
    filteredArray.reduce(
      (sum, raiting) => parseFloat(sum) + parseFloat(raiting["imdbRating"]),
      0
